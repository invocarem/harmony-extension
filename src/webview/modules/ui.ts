...
export function addMessage(
  ...
  if (contextSummary && isUser) {
    ...
    if (contextSummary.rulesCount !== undefined) {
      items.push(
        `<div class="context-summary-item"><span class="context-summary-label">Rules:</span> <span>${contextSummary.rulesCount}</span></div>`
      );
    }
    if (contextSummary.mcpToolsCount !== undefined) {
      items.push(
        `<div class="context-summary-item"><span class="context-summary-label">MCP Tools:</span> <span>${contextSummary.mcpToolsCount}</span></div>`
      );
    }
    if (items.length > 0) {
    ...
    if (contextSummary.files && contextSummary.files.length > 0) {
      ...
      filesDiv.innerHTML =
        '<span class="context-summary-label">Files:</span> ' +
        contextSummary.files
          .map(
            (file) =>
              `<span class="context-summary-file">📄 ${escapeHtml(file)}</span>`
          )
          .join("");
  ...
  if (verboseInfo && !isUser) {
    ...
    if (formattedText) {
      ...
      verboseDiv.innerHTML =
        '<div class="verbose-info-header">ℹ️ Verbose Info</div>';
  ...
  const shouldShowNoResponse =
    !isUser && !hasFinal && !formattedText && !verboseInfo;
  ...
  if (hasFinal) {
    ...
    finalDiv.innerHTML =
      '<div class="final-header">✨ Final Result</div>' + formatMarkdown(final);
...
export function updateLastUserMessageContextSummary(
  contextSummary: ContextSummary
): void {
  ...
  const existingContext =
    lastUserMessageElement.querySelector(".context-summary");
  if (existingContext) {
  ...
  if (contextDiv.children.length > 0) {
    ...
    const contentDiv = lastUserMessageElement.querySelector(
      "div:not(.context-summary):not(.timestamp)"
    );
    if (contentDiv) {
      lastUserMessageElement.insertBefore(contextDiv, contentDiv);
    } else {
      lastUserMessageElement.insertBefore(
        contextDiv,
        lastUserMessageElement.firstChild
      );
    }
  }
}
...
export function updateStreamingMessage(text: string): void {
  ...
  if (streamingMessageElement) {
    ...
    console.warn(
      "UI: streamingMessageElement is null after startStreamingMessage"
    );
  }
}
 ...
 * Optionally update it with the final complete content
 * If no streaming message exists, creates a new one
 */
export function finalizeStreamingMessage(finalContent?: {
  text?: string;
  reasoning?: string;
  commentary?: string;
  final?: string;
  ...
  if (streamingMessageElement) {
    ...
    // If final content provided, update the message with all sections
    if (finalContent) {
      updateStreamingMessageContent(streamingMessageElement, finalContent);
    }

    streamingMessageElement = null;
  } else if (finalContent) {
    // No streaming message exists, create a new one from the final content
    console.log(
      "UI: No streaming message to finalize, creating new message from final content"
    );
    addMessage(
      finalContent.text || "",
      false,
...
/**
 * Update streaming message element with complete content including reasoning, commentary, final
 */
function updateStreamingMessageContent(
  ...
  if (content.verboseInfo) {
    const verboseDiv = document.createElement("div");
    verboseDiv.className = "verbose-info-section";

    const verboseInfoWithToString = addToString(content.verboseInfo);
    const formattedText = verboseInfoWithToString.toString();

    if (formattedText) {
      const verboseContentDiv = document.createElement("div");
      verboseContentDiv.className = "verbose-info-content";
      verboseContentDiv.style.fontFamily = "monospace";
      verboseContentDiv.style.whiteSpace = "pre-wrap";
      verboseContentDiv.style.fontSize = "0.9em";
      verboseContentDiv.style.lineHeight = "1.4";
      verboseContentDiv.style.padding = "8px";
      verboseContentDiv.style.backgroundColor = "rgba(0, 0, 0, 0.05)";
      verboseContentDiv.style.borderRadius = "4px";
      verboseContentDiv.style.overflowX = "auto";
      verboseContentDiv.textContent = formattedText;

      verboseDiv.innerHTML =
        '<div class="verbose-info-header">ℹ️ Verbose Info</div>';
      verboseDiv.appendChild(verboseContentDiv);
      messageDiv.appendChild(verboseDiv);
    }
  }

  // Add reasoning section
  if (content.reasoning && content.reasoning.trim()) {
    const reasoningDiv = document.createElement("div");
    reasoningDiv.className = "reasoning-section";
    reasoningDiv.innerHTML =
      '<div class="reasoning-header">💭 Reasoning</div>' +
      formatMarkdown(content.reasoning);
    messageDiv.appendChild(reasoningDiv);
  }

  // Add commentary section
  if (content.commentary && content.commentary.trim()) {
    const commentaryDiv = document.createElement("div");
    commentaryDiv.className = "commentary-section";
    commentaryDiv.innerHTML =
      '<div class="commentary-header">💬 Commentary</div>' +
      formatMarkdown(content.commentary);
    messageDiv.appendChild(commentaryDiv);
  }

  // Format markdown and add content
  const hasFinal = content.final && content.final.trim();
  const displayText = hasFinal ? "" : content.text || "";
  const formattedMainContent = formatMarkdown(displayText);
  const mainContentDiv = document.createElement("div");
  const shouldShowNoResponse = !hasFinal && !formattedMainContent;
  mainContentDiv.innerHTML =
    formattedMainContent ||
    (hasFinal ? "" : shouldShowNoResponse ? "No response received." : "");
  messageDiv.appendChild(mainContentDiv);

  // Add final section if present
  if (hasFinal && content.final) {
    const finalDiv = document.createElement("div");
    finalDiv.className = "final-section";
    finalDiv.innerHTML =
      '<div class="final-header">✨ Final Result</div>' +
      formatMarkdown(content.final);
    messageDiv.appendChild(finalDiv);
  }
}
 ...
 */
export function updateStageIndicator(
  ...
  if (arrowChatToSnippet) {
    ...
    if (stage === "chat") {
      arrowChatToSnippet.classList.remove("disabled");
    } else {
  ...
  if (arrowSnippetToAssumptions) {
    // Enable if we're in snippet stage
    if (stage === "snippet") {
      arrowSnippetToAssumptions.classList.remove("disabled");
    } else {
  ...
  const buttonNextStep = document.getElementById(
    "button-next-step"
