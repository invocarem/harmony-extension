...
export function formatMarkdown(text: string): string {
  ...
  for (let i = 0; i < codeBlocks.length; i++) {
    ...
    // Use base64 encoding to preserve newlines and special characters in data attribute
    const base64Code = btoa(unescape(encodeURIComponent(code.trim())));
    const copyButton = `<button class="code-action-btn" data-action="copy" data-code-base64="${base64Code}" title="Copy code">📋</button>`;
