...
export class CodeContext {
  ...
  static fromCodeBlock(
    ...
    if (!codeBlockMatch) {
      ...
      const isValidFileName = (name: string): boolean => {
        ...
        const commonWords =
          /^(file|File|code|Code|script|Script|text|Text|data|Data)$/i;
      ...
      const versionMatch =
        firstLine.match(/\s+(v\d+(?:\.\d+)?|@v\d+(?:\.\d+)?)$/i) ||
        firstLine.match(/@(v\d+(?:\.\d+)?)$/i);
      if (versionMatch) {
        version = versionMatch[1].replace("@", "");
        const firstLineWithoutVersion = firstLine
          .replace(/\s+(v\d+(?:\.\d+)?|@v\d+(?:\.\d+)?)$/i, "")
          .trim();
        if (
          isValidFileName(firstLineWithoutVersion) &&
          altContent.split("\n").length > 1
        ) {
        ...
        isValidFileName(firstLine.trim()) &&
        altContent.split("\n").length > 1
      ) {
    ...
    if (
      ...
      !contentLines.some((line) => line.trim().length > 0)
    ) {
      return null;
    }
    ...
    if (!fileName) {
      ...
      const filePathMatch =
        beforeMatch.match(
          /(?:file|filename|path)[:\s]+`?([^\s`]+\.\w{2,4})`?/i
        ) ||
        beforeMatch.match(
          /\*\*(?:file|filename|path)\*\*[:\s]+`([^`]+\.\w{2,4})`/i
        ) ||
      ...
      const simpleFilenameMatchFirstLine = firstLineTrimmed.match(
        /^#\s*([a-zA-Z][\w\.-]*\.\w{2,4})(?:\s|$)/
      );
      // Pattern for trimmed content: # followed by optional space, filename, then space/newline/end/quote
      const simpleFilenameMatch = codeContent.match(
        /^#\s*([a-zA-Z][\w\.-]*\.\w{2,4})(?:\s|$|\n|")/m
      );

      // Also try matching with required space after # (more common format: "# hello.py")
      const simpleFilenameMatchWithSpaceFirstLine = firstLineTrimmed.match(
        /^#\s+([a-zA-Z][\w\.-]*\.\w{2,4})(?:\s|$)/
      );
      const simpleFilenameMatchWithSpace = codeContent.match(
        /^#\s+([a-zA-Z][\w\.-]*\.\w{2,4})(?:\s|$|\n|")/m
      );
      ...
      if (
        ...
        !simpleFilenameMatchFirstLine &&
        !simpleFilenameMatchWithSpaceFirstLine
      ) {
        console.log(
          `[CodeContext] First line starts with # but no match: "${firstLineTrimmed.substring(0, 30)}"`
        );
      }

      const codeBlockWithPath =
        simpleFilenameMatch ||
        simpleFilenameMatchWithSpace ||
        codeContent.match(
          /^#\s*(?:file\s+path|file|path)[:\s]+([^\n]+\.\w{2,4})/im
        ) ||
        codeContent.match(
          /^\/\/\s*(?:file\s+path|file|path)[:\s]+([^\n]+\.\w{2,4})/im
        ) ||
      ...
      if (simpleFilenameMatchFirstLine && simpleFilenameMatchFirstLine[1]) {
        ...
        simpleFilenameMatchWithSpaceFirstLine &&
        simpleFilenameMatchWithSpaceFirstLine[1]
      ) {
        ...
        simpleFilenameMatchWithSpace &&
        simpleFilenameMatchWithSpace[1]
      ) {
        ...
        codeBlockWithPath &&
        typeof codeBlockWithPath === "object" &&
        codeBlockWithPath[1]
      ) {
      ...
      if (extractedName) {
        ...
        if (firstLine && firstLine.trim().startsWith("#")) {
          console.log(
            `[CodeContext] No filename extracted from firstLine="${firstLine.substring(0, 50)}". Matches: firstLine=${!!simpleFilenameMatchFirstLine}, withSpace=${!!simpleFilenameMatchWithSpaceFirstLine}, content=${!!simpleFilenameMatch}`
          );
        }
      }
    }
  ...
  isReady(): boolean {
    return (
      ...
      this.content.some((line) => line.trim().length > 0)
    );
  }
