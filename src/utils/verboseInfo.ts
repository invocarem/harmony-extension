...
export class VerboseInfoBuilder {
    ...
    if (content) {
      ...
      if (sentences.length > 0) {
        ...
        for (let i = 0; i < Math.min(sentences.length, 3); i++) {
          ...
          if (lowerSentence.includes('tool') && lowerSentence.includes('call')) continue;
          if (lowerSentence.includes('i will') && lowerSentence.includes('use')) continue;
          if (lowerSentence.includes('let me') && lowerSentence.includes('check')) continue;
        ...
        if (candidateSentences.length > 0) {
          ...
          if (firstSentence.length >= 20) {
            ...
            const finalRestatement = restatement.match(/[.!?]$/) ? restatement : restatement + '.';
            return {
  ...
  static forAssumptionStage(
    ...
    const activeContexts = codeContexts.filter(cc => cc.isActive && cc.waitForCreate);
  ...
  static forImplementationStage(
      ...
      if (plan) {
        ...
        if (toolCalls) {
          toolCalls.forEach(tc => {
            const stepNum = tc.relatedStep || currentStep?.stepNumber || 
              (plan.steps.filter(s => s.status === 'completed').length > 0 
                ? plan.steps.filter(s => s.status === 'completed').length + 1 
                : 1);
        ...
        verboseInfo.planProgress = {
          ...
          currentStep: currentStep ? {
            ...
            startedAt: currentStep.status === 'in_progress' ? Date.now() : undefined,
            completedAt: currentStep.status === 'completed' ? Date.now() : undefined
          } : undefined,
    ...
    if (fileOperations) {
      const activeStepNumber = context?.progressPlan 
        ? (progressPlanManager.getPlan(context.progressPlan.taskId)?.steps.find(s => s.status === 'in_progress' || s.status === 'pending')?.stepNumber)
        : undefined;
...
export class VerboseInfoFormatter {
  ...
  private static formatAssumptionVerboseInfo(info: AssumptionVerboseInfo): string {
    const lines: string[] = [];
  ...
  private static formatImplementationVerboseInfo(info: ImplementationVerboseInfo): string {
    const lines: string[] = [];
...
export class ChatVerboseInfoDisplay implements ChatVerboseInfo {
  ...
  get stageTransition() { return this.info.stageTransition; }
  get step() { return this.info.step; }
  get maxSteps() { return this.info.maxSteps; }
  get isComplete() { return this.info.isComplete; }
  get problemSummary() { return this.info.problemSummary; }
  get extractedFiles() { return this.info.extractedFiles; }
  get toolCalls() { return this.info.toolCalls; }
}
...
export class AssumptionVerboseInfoDisplay implements AssumptionVerboseInfo {
  ...
  get maxSteps() { return this.info.maxSteps; }
  get isComplete() { return this.info.isComplete; }
  get codeSnippets() { return this.info.codeSnippets; }
  get progressPlan() { return this.info.progressPlan; }
  get toolCalls() { return this.info.toolCalls; }
}
...
export class ImplementationVerboseInfoDisplay implements ImplementationVerboseInfo {
  ...
  get maxSteps() { return this.info.maxSteps; }
  get isComplete() { return this.info.isComplete; }
  get planProgress() { return this.info.planProgress; }
  get fileOperations() { return this.info.fileOperations; }
  get toolCalls() { return this.info.toolCalls; }
}
