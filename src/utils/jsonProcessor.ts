...
export class JsonProcessor {
   ...
   */
  static extractToolCall(text: string): { name: string; arguments: any; raw: string } | null {
    try {
  ...
  /**
   * Extract all JSON tool calls from text
   */
  static extractAllToolCalls(text: string): Array<{ name: string; arguments: any; raw: string }> {
    ...
    while (lastIndex < text.length) {
      ...
      lastIndex += substring.indexOf(toolCall.raw) + toolCall.raw.length;
    }
