...
Fourth step with finalization label.`;
  ...
  describe("AutoTransitionManager.extractStepsFromText with merged steps bug", () => {
    ...
    it("should extract all 4 steps from the real-world LLM response", () => {
      const llmOutput = `**Implementation plan (one step per request)**
...
Here's my implementation plan. Should I proceed to the Implementation stage to execute it?`;
      ...
      const steps = manager.extractStepsFromText(llmOutput, undefined, "simple");
      
      console.log('AutoTransitionManager extracted steps:', JSON.stringify(steps, null, 2));
      
      // Should extract at least 4 steps (complexity=simple might add fallback, but we want the parsed steps)
      expect(steps.length).toBeGreaterThanOrEqual(4);
      
      // Check that the steps are properly separated
      const step1 = steps.find(s => s.goal.includes("Step 1"));
      const step2 = steps.find(s => s.goal.includes("Step 2"));
      const step3 = steps.find(s => s.goal.includes("Step 3"));
      const step4 = steps.find(s => s.goal.includes("Step 4"));
      ...
      // Step 2 should NOT contain the text from Steps 3 and 4
      if (step2) {
        expect(step2.goal).not.toContain("Step 3");
        expect(step2.goal).not.toContain("Step 4");
        expect(step2.description).not.toContain("Wrap the conversion");
        expect(step2.description).not.toContain("After making the above changes");
      }
    });
  });
});
