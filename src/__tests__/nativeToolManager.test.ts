import { NativeToolsManager, NativeTool, NativeToolResult } from '../nativeToolManager';
...
describe('NativeToolsManager', () => {
  ...
  describe('getAvailableTools', () => {
    ...
    it('should have correct schema for find_files tool', () => {
      ...
      expect(findFilesTool?.description).toContain('Find files by name pattern');
      ...
      expect(findFilesTool?.inputSchema.properties.case_sensitive).toBeDefined();
    ...
    it('should have correct schema for list_files tool', () => {
      ...
      expect(listFilesTool?.inputSchema.properties.directory_path).toBeDefined();
      expect(listFilesTool?.inputSchema.properties.recursive).toBeDefined();
      expect(listFilesTool?.inputSchema.properties.include_hidden).toBeDefined();
    });
  });
  ...
  describe('read_file', () => {
    it('should read file successfully', async () => {
      ...
      const result = await manager.callTool('read_file', { file_path: filePath });
      
      expect(result.isError).toBeUndefined();
      expect(result.content).toHaveLength(1);
      expect(result.content[0].text).toBe(content);
      expect(mockFs.promises.readFile).toHaveBeenCalledWith(resolvedPath, 'utf-8');
    });
